...
import { PatientDtoV2 } from '../types';
...
describe('Patients API V2 Integration Tests', () => {
  describe('GET /api/v2/patients', () => {
    it('should return list of patients with statistics', async () => {
      const patients = await apiClient.get<PatientDtoV2[]>(endpoints.patientsV2);

      expect(patients).toBeDefined();
      expect(Array.isArray(patients)).toBe(true);

      console.log(`[${config.dbProvider}] Fetched ${patients.length} patients from V2 API`);
    });
    ...
    it('should return patients with correct V2 structure including statistics', async () => {
      ...
      if (patients.length > 0) {
        ...
        console.log(`[${config.dbProvider}] V2 Patient with statistics:`, patient);
      }
    });
    ...
    it('should have statistics with correct data types', async () => {
      ...
      if (patients.length > 0) {
        ...
        console.log(`[${config.dbProvider}] V2 Statistics validated:`, stats);
      }
    });
  });
  ...
  describe('GET /api/v2/patients/:id', () => {
    it('should return a single patient by id with statistics', async () => {
      const allPatients = await apiClient.get<PatientDtoV2[]>(endpoints.patientsV2);

      if (allPatients.length > 0) {
        const firstPatientId = allPatients[0].id;
        const patient = await apiClient.get<PatientDtoV2>(endpoints.patientByIdV2(firstPatientId));

        expect(patient).toBeDefined();
        expect(patient.id).toBe(firstPatientId);
        expect(patient).toHaveProperty('statistics');

        console.log(`[${config.dbProvider}] Fetched patient V2 with ID ${firstPatientId}:`, patient);
      }
    });
    ...
    it('should return 404 for non-existent patient', async () => {
      ...
      try {
        await apiClient.get<PatientDtoV2>(endpoints.patientByIdV2(nonExistentId));
  ...
  describe('V1 vs V2 Compatibility', () => {
    it('should have V2 as superset of V1 (all V1 fields present in V2)', async () => {
      const patientsV1 = await apiClient.get<any[]>(endpoints.patientsV1);
      const patientsV2 = await apiClient.get<any[]>(endpoints.patientsV2);
      ...
      if (patientsV1.length > 0 && patientsV2.length > 0) {
        ...
        console.log(`[${config.dbProvider}] V1 fields:`, v1Fields);
        console.log(`[${config.dbProvider}] V2 fields:`, v2Fields);
      }
    });
  });
});
