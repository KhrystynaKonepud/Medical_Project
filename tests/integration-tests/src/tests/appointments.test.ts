...
describe('Appointments API Integration Tests', () => {
  describe('GET /api/appointments', () => {
    it('should return list of appointments', async () => {
      const appointments = await apiClient.get<Appointment[]>(endpoints.appointments);

      expect(appointments).toBeDefined();
      expect(Array.isArray(appointments)).toBe(true);

      console.log(`[${config.dbProvider}] Fetched ${appointments.length} appointments`);
    });
    ...
    it('should return appointments with correct structure', async () => {
      ...
      if (appointments.length > 0) {
        ...
        console.log(`[${config.dbProvider}] Appointment structure validated:`, {
          id: appointment.id,
    ...
    it('should have valid appointment status', async () => {
      ...
      console.log(`[${config.dbProvider}] All appointments have valid status values`);
    });
    ...
    it('should have valid date format', async () => {
      ...
      console.log(`[${config.dbProvider}] All appointments have valid date format`);
    });
    ...
    it('should include patient and doctor relations', async () => {
      ...
      if (appointments.length > 0) {
        ...
        console.log(`[${config.dbProvider}] Appointment relations validated:`, {
  ...
  describe('GET /api/appointments/:id', () => {
    it('should return a single appointment by id', async () => {
      const allAppointments = await apiClient.get<Appointment[]>(endpoints.appointments);

      if (allAppointments.length > 0) {
        const firstAppointmentId = allAppointments[0].id;
        const appointment = await apiClient.get<Appointment>(endpoints.appointmentById(firstAppointmentId));
        ...
        console.log(`[${config.dbProvider}] Fetched appointment with ID ${firstAppointmentId}:`, appointment);
      }
    });
    ...
    it('should return 404 for non-existent appointment', async () => {
      ...
      try {
        await apiClient.get<Appointment>(endpoints.appointmentById(nonExistentId));
        fail('Expected 404 error but request succeeded');
      } catch (error: any) {
        expect(error.response?.status).toBe(404);
        console.log(`[${config.dbProvider}] Correctly returned 404 for appointment ID ${nonExistentId}`);
      }
    });
  });
  ...
  describe('Appointment Data Integrity', () => {
    it('should have matching patient and doctor IDs in relations', async () => {
      ...
      console.log(`[${config.dbProvider}] All appointment relations have matching IDs`);
    });
    ...
    it('should have non-empty reason', async () => {
      ...
      console.log(`[${config.dbProvider}] All appointments have non-empty reasons`);
    });
  });
});
